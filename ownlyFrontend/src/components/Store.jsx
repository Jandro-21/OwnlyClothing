...
export default function Store() {
  ...
  // Cargar productos y clasificarlos
  useEffect(() => {
  const fetchProducts = async () => {
    try {
      ...
      const classifiedProducts = data.map(p => {
        let assignedCategory = 'Accesorios'; // Por defecto
        const name = p.name.toLowerCase();

        // Filtro mejorado (Español + Inglés para Printful)
        if (
          ...
          assignedCategory = 'Sudaderas';
        } else if (
          name.includes('pantalón') || 
          name.includes('pant') || 
          name.includes('shorts') || 
          name.includes('chándal') || 
          name.includes('jogger')
        ) {
          assignedCategory = 'Pantalones';
        } else if (
          name.includes('gorra') || 
          name.includes('cap') || 
          name.includes('hat') || 
          name.includes('beanie') || 
          name.includes('visera')
        ) {
          assignedCategory = 'Gorras';
        }

        return {
          ...p,
          mappedCategory: assignedCategory
        };
      });
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      else alert("Hubo un problema al generar el enlace de pago.");
    } catch (error) {
      console.error(error);
  ...
  // Categorías exactas que pide el usuario
  const categories = ['Camisetas', 'Sudaderas', 'Pantalones', 'Gorras', 'Accesorios'];
  
  // Filtrado por el 'mappedCategory' que creamos en el useEffect
  const filteredProducts = currentCategory === 'All' 
    ? products 
    : products.filter(p => p.mappedCategory === currentCategory);
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <header className="glass-panel sticky top-0 z-40 mx-4 mt-4 mb-6 rounded-2xl px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => { setView('store'); setCurrentCategory('All'); }}>
        ...
        <div className="flex gap-4">
          <button onClick={() => setIsCartOpen(true)} className="glass-button p-2 rounded-full relative">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 flex flex-col md:flex-row gap-6">
        ...
        <aside className="w-full md:w-64 h-fit sticky top-28 glass-panel rounded-2xl p-6">
          ...
          <nav className="flex flex-col space-y-2 mb-6">
            <button 
              ...
              className={`text-left px-4 py-2 rounded-xl transition-colors ${currentCategory === 'All' ? 'bg-purple-600/20 text-purple-600 font-bold' : 'hover:bg-white/30 dark:hover:bg-white/10'}`}
            >
              Todas
            </button>
            {categories.map(cat => (
              <button 
                ...
                className={`text-left px-4 py-2 rounded-xl transition-colors ${currentCategory === cat ? 'bg-purple-600/20 text-purple-600 font-bold' : 'hover:bg-white/30 dark:hover:bg-white/10'}`}
              >
                {cat}
              </button>
            ))}
        ...
        <main className="flex-1">
          {view === 'store' ? (
            <div>
              <h2 className="text-2xl font-bold mb-6 px-2">{currentCategory === 'All' ? 'Todos los Productos' : currentCategory}</h2>
              {filteredProducts.length === 0 ? (
                <p className="text-center text-gray-500 dark:text-gray-400 py-10">No hay productos disponibles en esta categoría actualmente.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredProducts.map(p => (
                    <div key={p.id} className="glass-panel p-6 rounded-2xl text-center flex flex-col items-center">
                      ...
                      <h3 className="font-bold text-lg line-clamp-2 min-h-[3.5rem]">{p.name}</h3>
                      <p className="text-purple-600 dark:text-purple-400 font-bold mb-4">{p.retail_price ? `${p.retail_price}€` : '20.00€'}</p>
                      
                      <button onClick={() => addToCart(p)} className="glass-button w-full py-2 rounded-xl flex justify-center items-center gap-2 font-semibold hover:bg-purple-500 hover:text-white transition-colors mt-auto">
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm transition-opacity">
          <div className="w-full max-w-md h-full bg-white dark:bg-gray-900 shadow-2xl p-6 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsCartOpen(false)} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-800"><X className="w-6 h-6" /></button>
            </div>
                  ...
                  <div key={index} className="flex justify-between items-center p-4 border border-gray-200 dark:border-gray-800 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-purple-600 font-bold">{item.retail_price || '20.00'}€</p>
                      </div>
                    </div>
            ...
            {cart.length > 0 && (
              <div className="border-t border-gray-200 dark:border-gray-800 pt-6 mt-4">
                ...
                <button onClick={handleStripeCheckout} disabled={isCheckingOut} className={`w-full bg-[#635BFF] text-white py-4 rounded-xl font-bold flex justify-center items-center gap-2 transition-colors ${isCheckingOut ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[#4b45cf]'}`}>
                  <CreditCard className="w-5 h-5" /> {isCheckingOut ? 'Procesando...' : 'Pagar de forma segura'}
                </button>
              </div>
            )}
          </div>
...
function Customizer({ onBack }) {
  ...
  const handleSendToPrintful = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if(data.success) alert("Diseño guardado temporalmente.");
    } catch (e) {
